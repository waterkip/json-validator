...
sub _new_schema {
  ...
  push @attrs, $self->{formats} if $self->{formats};
  my $schema = $self->_schema_class($spec)
    ->new($spec, @attrs, store => $self->store, version => $self->{version});
...
sub _resolve {
  ...
  if (ref $schema eq 'HASH') {
    ...
    $self->store->add_schema($schema, $schema);
    return $schema;
  }
  else {
    ($schema, $id) = $self->store->load_schema($schema);
